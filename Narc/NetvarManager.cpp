...
//------------------------------------------------------------
unique_ptr<NetvarTable> NetvarManager::InternalLoadTable(SSDK::RecvTable* pRecvTable, uint32_t offset) {
	...
	for (auto i = 0; i < pRecvTable->m_nProps; ++i) {
		...
		if (pProp->m_RecvType == SSDK::SendPropType::DPT_DataTable &&  //If it is a table AND
			pProp->GetDataTable() != NULL &&                                   //The DataTable isnt null AND
			pProp->GetDataTable()->m_pNetTableName[0] == 'D') {                //The Table name starts with D (this is because there are some shitty nested 
																			   //tables that we want to skip, and those dont start with D)

																			   //Load the table pointed by pProp->m_pDataTable and insert it
			pTable->insert_table(pProp->m_pVarName, InternalLoadTable(pProp->GetDataTable(), pProp->GetOffset()));
		}
		else {
			pTable->insert_prop(pProp->m_pVarName, pProp->GetOffset());
		}
		m_netvarCount++;
	}
	return pTable;
